...
public interface ProductRepository extends JpaRepository<Product,Long> {
    ...
    @Query(value = "SELECT * FROM product p JOIN product_category_rel pl ON pl.p_id = p.id WHERE pl.c_id=?1 AND p.price > ?2", nativeQuery = true)
    List<Product> retrieveProductListByCategoryAndPriceGreaterThan(Long categoryId, BigDecimal price);
