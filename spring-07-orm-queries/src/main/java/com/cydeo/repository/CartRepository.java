...
public interface CartRepository extends JpaRepository<Cart,Long> {
    ...
    @Query(value = "SELECT * FROM cart ct JOIN customer c ON ct.customer_id=c.id WHERE ct.customer_id=?1 AND ct.cart_state=?2 AND ct.discount_id IS NOT NULL",nativeQuery = true)
    List<Cart> retrieveAllCartByCustomerAndCartStateAndDiscountIsNotNull(@Param("id") Long id, @Param("cartState") String cartState);
